...
// I should ultimately make curDisp a local variable passed to display
    function display() {
      const dlight=['RedLight.png','GreenLight.png'];
      const xsig = [];
      xsig[0]=document.querySelector('#tSigG');
      xsig[1]=document.querySelector('#rSigG');
      xsig[2]=document.querySelector('#bSigG');
      xsig[3]=document.querySelector('#lSigG');
      let tsig = xsig[0];
      if (curDisp[0] === 'G') {
        xsig[0].src = dlight[0];
      } else { 
...
if (randimg === 0 ) {
  document.getElementById("dCenter").style.transition="all 2s"
